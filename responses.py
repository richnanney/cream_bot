...
def next_birthday():
    for person in people:
        if today.month <= int(person.month) and today.day < int(person.day):
            ...
            ordinal = "th"
            if str(person.day) == "11" or str(person.day)== "12" or str(person.day)== "13":
                ordinal = "th"
            ...
            message = "The next birthday is " + person.name + "'s, which will be on " + next_birthday_month + " " + person.day + ordinal + ". " + person.pronoun + " will be turning " + next_birthday_age + "."
            return message
    else:
...
def wish_birthday():
    ...
    for person in people:
        if today.month == person.month and today.day == person.day:
            ...
            message = "Wow @everyone! Today is our very own's " + birthday_boy + " birthday. Please wish them a happy birthday!"
            return message
    return False
...
def list_birthdays():
    birthdays = ""
    for person in people:
        message = person.name + "'s birthday is on " + person.month + "/" + person.day + ".\n"
        birthdays += message
    return birthdays
